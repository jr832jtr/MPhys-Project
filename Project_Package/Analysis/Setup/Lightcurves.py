...
from . import HostGalaxies
from . import MyCosmology
from . import MyAstroCalc

def lognorm_lightcurve(t_max, deltat, stretch=1, s=1, scale=1, loc=None, peakatone=False, norm=1, 
                       ttype = 1, Time = None, t_min = None):
    if ttype == 1:
        if not t_min:
            t = numpy.arange(0, int(t_max), deltat)
        elif t_min:
            t = numpy.arange(int(t_min), int(t_max), deltat)
    elif ttype == 2:
        t = Time#numpy.arange(Time[0], max(Time), abs(Time[1] - Time[0]))
...
def burst_lognorm(t_max, deltat, burstheight, burstlength, bursttime, peakatone=True, t_min = None):
    """
    ...
    if not t_min:
        ...
        t = numpy.arange(t_min, t_max, deltat)
    f = numpy.zeros_like(t)
    if type(burstheight) in [float, int]:
        burstheight = [burstheight]
    ...
    for mu, sigma, scale in zip(bursttime, burstlength, burstheight):
        f += lognorm_lightcurve(t_max, deltat, loc=mu, stretch=sigma, norm=scale, peakatone=peakatone, t_min = t_min)[1]
    return t, f
...
def random_burst(t_max, deltat, downtime, burstlength, burstheight, bursttype='gauss',
                 randomheight=False, randdist='lognorm', randpars=None, scalecutoff=0.01, 
                 peakatone=True, t_min = None, BurstTimes = False):
    """
    ...
    if not t_min:
        ...
        bursttimes = numpy.random.uniform(t_min, t_max, n_burst)
