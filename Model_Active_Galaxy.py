...
class Active_Galaxy():
    ...
    def __init__(self, agnparams, Sim, gal_params, obs_wavs, gal_no, fraction = 0.5, Thresh = 25, Scaling = 1e44, Show_Lc = False):
        ...
        self.AGN_Params = {'TimesMasses':self.Galaxies['TimesMasses'], 'AGN_Params':agnparams}
        
        if self.Agn_Type != 'Delay':
            self.Active_Galaxies, self.TriggerTimes = NewFunctions.Generate_AGN(self.Galaxies['Time'], self.Galaxies['lognormlist'], self.Galaxies['AccRates'], self.AGN_Params['AGN_Params'])
        elif self.Agn_Type == 'Delay':
            self.Active_Galaxies = NewFunctions.Generate_AGN(self.Galaxies['Time'], self.Galaxies['lognormlist'], self.Galaxies['AccRates'], self.AGN_Params['AGN_Params'])
            self.TriggerTimes = [[agnparams['T_Delay']]]*len(self.Galaxies['lognormlist'])
        else:
    ...
    def SFHs(self, Obs_Wavs, Sim, Fraction, Params):
        return NewFunctions.Generate_SFHs(Obs_Wavs, Sim, Fraction, Params)
    
    
    
    def Generate_Spectra(self, Number, Thresh, Scaling, show):
        AGN_ON_Spectras = []
        AGN_ON = NewFunctions.AGN_Periods(self.Agn_Type, self.Active_Galaxies, self.TriggerTimes, Number, Thresh, Scaling, self.Galaxies['Galaxy'].sfh.age_of_universe, show = show)
        ...
        for i in range(len(AGN_ON)):
            for j in range(len(AGN_ON[i])):
                ...
                try:
                    mass_frac = NewFunctions.Mass_Calculator(self.Gal_Params['redshift'], AGN_ON[i][j], self.Galaxies, Number, 100, T_min = AGN_ON[0][0] - 4e8)
                except ValueError:
                    break
